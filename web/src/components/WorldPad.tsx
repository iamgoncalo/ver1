...
// step between the five worlds, the center pill is Home/world "0" - same
...
export function WorldPad({ world, onSelect, onGoHome }: { world: number; onSelect: (n: number) => void; onGoHome: () => void }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 5, padding: 3, borderRadius: 12, border: "1px solid var(--line)", background: "var(--surface-2)" }}>
      <PadButton dir="left" disabled={world <= 1} onClick={() => onSelect(Math.max(1, world - 1))} />
      <button
        onClick={onGoHome}
        title="Jump to the Innovation Funnel home screen"
        aria-label="Home"
        className="mono"
        style={{
