...
def build():
    ...
    for p in magic_box["possibilities"]:
        ...
        concepts.append({
            ...
            # Pass 2 lineage - carried through verbatim from the Magic Box
            # possibility so the concept surface never loses its parents.
            "possibility_id": p.get("possibility_id", p["id"]),
            "target_category": p.get("target_category"),
            "parent_path_ids": p.get("parent_path_ids", []),
            "source_evidence_ids": p.get("source_evidence_ids", []),
            "friction_ids": p.get("friction_ids", []),
            "operator_origin": p.get("operator_origin"),
            "donor_capability_ids": p.get("donor_capability_ids", []),
            "donor_state": p.get("donor_state"),
            "assumption_challenged": p.get("assumption_challenged"),
            "why_here": p.get("why_here"),
            "product_archetype": p.get("product_archetype"),
            "engineering_envelope": p.get("engineering_envelope"),
            "unknowns": p.get("unknowns", []),
            "test": p.get("test"),
            "economic_value_caveat": p.get("economic_value_caveat"),
            "comparable_market_median_usd": p.get("comparable_market_median_usd"),
            "comparable_market_median_n_products": p.get("comparable_market_median_n_products"),
            "comparable_market_median_caveat": p.get("comparable_market_median_caveat"),
