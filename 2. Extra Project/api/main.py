...
def lab_state(os_id: str = "OS-1"):
    ...
    actually exists - a disclosure PDF on disk means CONCEPT; nothing is
    asserted that isn't a real file."""
    if not os_id.replace("-", "").isalnum():
        raise HTTPException(status_code=400, detail="invalid os_id")
    disclosure = os.path.join(WEB_DIST, "innovation-disclosures", f"{os_id}.pdf")
    artifacts = []
    if os.path.isfile(disclosure):
        artifacts.append({
            "id": f"disclosure-{os_id}", "kind": "concept_document",
            "title": f"Innovation Disclosure — {os_id}.pdf", "state": "generated",
            "path": f"/innovation-disclosures/{os_id}.pdf",
            "note": "Patent-style concept document, regenerated by every pipeline run.",
            "generated_at": __import__("datetime").datetime.utcfromtimestamp(os.path.getmtime(disclosure)).isoformat() + "Z",
        })
    ev_table = os.path.join(ROOT, "deliverables", "evidence_table.csv")
    if os.path.isfile(ev_table):
        artifacts.append({
            "id": "evidence-table", "kind": "evidence_table",
            "title": "Evidence table (JSON)", "state": "generated",
    ...
    return {
        ...
        "prototype": {
            ...
            "note": "States are earned by real files/work; a concept document exists when its PDF does.",
        },
        "artifacts": artifacts,
    }
